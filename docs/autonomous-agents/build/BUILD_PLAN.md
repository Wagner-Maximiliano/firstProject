...
> **Working codename:** **Helm** — the orchestration layer we build *above* Hermes. (Rename freely; it's only a label.)

---

## 1. What we're building (and what "V1 pilot" means)

**Helm** is the orchestration layer that drives a software project from idea to shipped product using a team of AI agents on top of **Hermes** (`hermes-agent`), with the human involved only at planning/sign-off, GUI testing, and go-live. The full design is in `../FRAMEWORK_SPEC.md`.
...
2. Helm turns the plan into a task DAG and drives a **single stream**: Builder → empirical + adversarial checks → Reviewer → PR merged to a trunk-protected `main`.
...
**Pilot success criterion (the demo):** from a one-paragraph idea, Helm ships a working tiny app (e.g. a "personal links page" or "to-do list with a single screen") to a staging URL, with the human only: signing off the plan, passing the GUI test, and approving go-live.
...
| Orchestration core | **Our own small Python package** (`helm/`) | The business rules (DAG, scheduling, escalation, board, watchdog) stay ours — not captive to a framework. |
...
| Config | **YAML** (`config/helm.yaml`) | Tier→provider map, thresholds, vendors, budgets. |
...
```
helm/                      # the orchestration package (our code)
  __init__.py
...
tests/                     # pytest suite for Helm itself
.github/workflows/         # CI
...
- *Deliverable:* `docs/adr/ADR-0001-hermes-integration.md` — actual capabilities (provider config, task slots, session persistence, compression, retries), how Helm will call it, and any deltas from `FRAMEWORK_SPEC §16`.
...
- *Deliverable:* repo structure (§3), `.venv` setup script, `pyproject.toml`, ruff/black/mypy config, pre-commit, empty `helm/` package, `README` for developers.
...
- *Deliverable:* `helm/gateway/` with config-driven mapping (T1 free, T2, T3, Board seats A/B/C across 3 vendors), a **mock provider**, and a smoke test per configured provider.
...
- *Deliverable:* `helm/state/` SQLite schema (tasks/DAG, leases, heartbeats, quota meters, append-only event log) + checkpoint/resume helpers.
...
- *Deliverable:* `helm/human/telegram.py` — send a message with inline option buttons, receive the choice; graceful no-op if token absent.
...
- *Deliverable:* `helm/watchdog/quota.py` — per-vendor rolling-window meter, soft-threshold downgrade to free, window-reset auto-resume.
...
- *Deliverable:* `helm/human/planning.py` + `prompts/planner.md`, `prompts/concierge.md`. Concierge (T1) triages; Architect (T3) drives **business-impact domains** with dive-deeper/trust controls; emits PRD + tech spec + **task DAG** + acceptance criteria + ADRs; presents an assumption ledger; waits for human sign-off via Telegram/CLI.
...
- *Goal:* Drive the DAG.
- *Deliverable:* `helm/orchestrator/` — load DAG, pick the next ready task, move Kanban cards (scripted), event-driven (consume CI/PR events; no model-polling).
...
- *Deliverable:* `helm/agents/builder.py` + `prompts/builder.md` — on a `task/<id>` branch, write code + tests + why-comments, open a draft PR.
...
- *Deliverable:* `helm/quality/` — T0 runner (tests/lint/build) with the "N failures in a row → escalate" rule; **inverted reviewer** (T1) producing a skepticism score; the escalation ladder.
...
- *Deliverable:* `helm/agents/reviewer.py` (T2, ≠ builder model) + `helm/orchestrator/integrator.py` — review PR, on approval+green merge via PR, delete branch, move card to Done.
...
- *Deliverable:* `helm/human/test_guide.py` — generate the 5-part guide (what/run/check/expect/if-wrong), boot an isolated `.venv` test env with seeded data, post to Telegram, collect pass/fail.
...
- *Deliverable:* `helm/board/` + `prompts/board_member.md` — rubric scoring (cost/security/maintainability, 1–10), mandatory floors, **prove-it-or-lose-it** vetoes, ADR output; convened only on §7.1 triggers.
...
- *Deliverable:* `helm/watchdog/` — heartbeats, stale detection, recovery ladder (retry → failover via gateway → requeue → escalate to Telegram).
...
- *Deliverable:* run Helm on the pilot toy idea start→finish; capture gaps in `docs/autonomous-agents/build/PILOT_REPORT.md`; fix blockers; list deferred items.
...
Provider keys (Anthropic/OpenAI/3rd) + Telegram bot token (via env secrets, not chat) · default models per tier + 3 board vendors · the pilot toy idea (or let the first planning run choose one) · escalation patience window. All have defaults in `config/helm.yaml`; the build proceeds on mocks until keys arrive.
